...
public class PDLFormatter extends AbstractFormatter2 {
  ...
  protected void _format(final simplepdl.Process process, @Extension final IFormattableDocument document) {
    ...
    EList<Ressource> _ressources = process.getRessources();
    for (final Ressource ressource : _ressources) {
      document.<Ressource>format(ressource);
    }
  }
  
  protected void _format(final WorkDefinition workDefinition, @Extension final IFormattableDocument document) {
    EList<parametre> _parametres = workDefinition.getParametres();
    for (final parametre parametre : _parametres) {
      document.<simplepdl.parametre>format(parametre);
    }
  }
