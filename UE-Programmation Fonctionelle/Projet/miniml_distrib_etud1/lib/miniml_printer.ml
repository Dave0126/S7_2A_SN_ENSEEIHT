...
let rec print_expr fmt expr =
  ...
  (* en fonctions                   *)
  | EBinop (_, VIRG, _)           -> Format.fprintf fmt "(fun x -> (fun y -> (x,y)))"
  | EBinop (_, CONS, _)           -> Format.fprintf fmt "(fun x -> (fun y -> (x::y)))"
  (* les autres opérateurs binaires *)
  (* sont banalisés en fonctions    *)
  | EBinop (e1,op,e2)              -> Format.fprintf fmt "( %a %a %a )" print_expr e1 print_binop op print_expr e2
