  ...
  struct
    ...
    let scheduler proc_init =
      ...
      let rec handler result =
        match result with
        | Done        -> if Queue.is_empty queue then () else handler (Queue.pop queue ())
        | Yield k     -> Queue.push k queue; handler (Queue.pop queue ())
        | Fork (p, k) -> Queue.push k queue; run p
      and run proc =
        handler (Delimcc.push_prompt prompt0 (fun () -> proc (); Done))
      in run proc_init;;
...
module GTChannel : Channel =
  struct
    let create () =
      let queue = Queue.create () in
      (fun v -> (Queue.push v queue; GreenThreads.yield ())),
      (let rec loop () =
         if not (Queue.is_empty queue)
         then Queue.pop queue
         else (GreenThreads.yield (); loop ())
       in loop);;
  end
...
let ping_pong () =
  GreenThreads.(
    let proc () =
      begin
        fork (fun () -> for i = 1 to 10 do Format.printf "ping !@ "; yield () done);
        fork (fun () -> for i = 1 to 10 do Format.printf "pong !@ "; yield () done);
        exit ()
      end
    in scheduler proc 
  ) 
