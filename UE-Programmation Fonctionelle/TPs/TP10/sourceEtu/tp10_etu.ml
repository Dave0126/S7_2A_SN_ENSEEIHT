...
let rec parse_P : (char, prog) parser = fun flux ->
  (
    (p_begin >>= fun () -> parse_I >>= fun insts -> p_end >>= fun() -> return insts)
    (** ou *)
  ) flux
and parse_I : (char, inst) parser = fun flux ->
  (
    (return [])
    ++ (** ou *)
    (parse_C >>= fun insts1-> p_ptvirg >>= fun () -> parse_I >>= fun insts2 -> return (insts1::insts2))
  ) flux
and parse_C : (char, cmd) parser = fun flux ->
  (
    (p_repeat >>= fun () -> p_entier >>= fun c -> parse_P >>= fun parse -> return (Repeat(c,parse)))
    ++ (** ou *)
    (p_repeat >>= fun () -> p_entier >>= fun c -> return (Repeat c))
    ++ (** ou *)
    (p_turn >>= fun() -> p_entier >>= fun c -> return (Turn c))
    ++ (** ou *)
    (p_on >>= fun () -> retrun (On))
    ++ (** ou *)
