...
// pool fixe
    ...
    public	Integer call() { // le résultat doit être un objet
        System.out.println("子进程正在运行中。");
        int ml = 0;
// ********* A compléter
        ml = tableau[début];
        for (int i = début; i <=fin; i++){
            ml = Math.max(ml, tableau[i]);
        }
        return ml;
    }
}
...
// pool fork-join
    ...
    protected Integer compute() {
        ...
        if (taille > seuil){
            System.out.println("Decomposition de taille : " + taille);

            TraiterFragment tf1 = new TraiterFragment(tableau, début, taille/2);
            TraiterFragment tf2 = new TraiterFragment(tableau, (taille/2+1), fin);

            // On fait un fork
            tf1.fork();
            tf2.fork();

            // On join sur les resultats
            if (tf1.join() >= tf2.join()){
                max = tf1.join();
            }
            else{
                max = tf2.join();
...
//-------- Pool Fixe
    ...
    throws InterruptedException, ExecutionException {
        List<Future<Integer>> résultats=new LinkedList<Future<Integer>>();// 有关线程池
        ...
        int grain = Math.max(1,t.length/nbT); 	/*taille d'une recherche locale 
                                                 ...
                                                 * 将一个线程池分成若干块，这是这一块的大小，>= 1 */
// ********* A compléter (initialisation des variables)
        int d;							//indice de départ d'une recherche locale: 它分成了好几块，这是这一块的开头
        int f;  						//indice de fin d'une recherche locale: 它分成了好几块，这是这一块的结尾
// ********* A compléter
        for (int i =0 ; i < nbT ; i=+grain){
            
            //soumettre les tâches
            Future<Integer> r = xs.submit(new MaxLocal(t, i, i+grain));
            résultats.add(r);
            //récupérer les résultats
            System.out.println("The max of this block of pool is " + r.get());
